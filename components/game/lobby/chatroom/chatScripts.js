...
$(document).ready(function(){
  ...
  var chat = $('#submit-lobby-chat');
  chat.submit(function(e) {
    e.preventDefault();
    var msg = $('#lobby-msg-box').val();
    if (msg === '') {
      return false;
    }
    var chatRoomId = $('#lobby-chat-room').attr('chatRoomId');

    socket.emit('message room', {username: USERNAME, msg: msg, roomId: chatRoomId});
  ...
  socket.on('message', function (data) {
    ...
    console.log(data);
    $('#lobby-msgs').append($('<span class="chat-message">').text(data.username + ': ' + data.msg));
    $('#lobby-msgs').append($('<br>'));
    $('#lobby-msgs').scrollTop($('#lobby-msgs')[0].scrollHeight);
